...
/**
 * @title CallbackTestRSC
 * @notice RSC that emits a Callback event on every react() call
 */
contract CallbackTestRSC is IReactive, AbstractReactive {
    ...
    constructor() payable {
        if (!vm) {
            ...
            service.subscribe(
                SEPOLIA_CHAIN_ID,
                VAULT,
                0xcfc791d57cbe67b39a1e34c851c3eebf9a7edb42f31260acf4f18a7d5959ff62, // YieldSnapshot topic
                REACTIVE_IGNORE,
                REACTIVE_IGNORE,
                REACTIVE_IGNORE
            );
