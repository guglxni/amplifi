...
contract Funder {
    ...
    /// @notice Reactive Faucet on Sepolia - converts SepETH to REACT at 100:1 ratio
    /// @dev Official faucet: request(address) sends REACT to specified address on Lasna
    address payable public constant REACTIVE_FAUCET = payable(0x9b9BB25f1A81078C544C829c5EB7822d747Cf434);
    ...
    /**
     * @notice Bridge funds via Reactive Faucet to get REACT tokens
     * @dev Uses official faucet: 1 SepETH = 100 REACT
     *      Max 5 SepETH per request (will get 500 REACT)
     * @param recipient Address to receive REACT on Lasna
     * @param amount Amount of SepETH to convert (max 5 ether)
     */
    function bridgeToFaucet(address recipient, uint256 amount) external onlyAuthorized {
        require(recipient != address(0), "Funder: invalid recipient");
        require(amount >= MIN_TRANSFER_AMOUNT, "Funder: amount too small");
        require(amount <= 5 ether, "Funder: max 5 ETH per faucet request");
        
        uint256 balance = address(this).balance;
        require(balance >= amount + gasReserve, "Funder: insufficient balance");
        
        // Call faucet's request(address) function
        // This converts SepETH to REACT at 100:1 ratio
        (bool success, ) = REACTIVE_FAUCET.call{value: amount}(
            abi.encodeWithSignature("request(address)", recipient)
        );
        
        if (success) {
            totalBridged += amount;
            bridgeCount++;
            emit FundsBridged(recipient, amount);
        } else {
            emit BridgeFailed(recipient, amount, "Faucet request failed");
            revert("Funder: faucet bridge failed");
        }
    }
    
    /**
     * @notice Auto-refill: Bridge to faucet for RSC gas funding
     * @dev Convenience function - bridges to targetRsc
     * @param amount Amount of SepETH to convert
     */
    function autoRefillReact(uint256 amount) external onlyAuthorized {
        require(targetRsc != address(0), "Funder: no target RSC");
        require(amount <= 5 ether, "Funder: max 5 ETH per faucet request");
        ...
        if (success) {
            ...
            revert("Funder: auto-refill failed");
        }
    }
